...
all:
	$(CC) $(COMPILER_FLAGS) $(SRC_FILES) $(INCLUDE_SDL_HEADERS) $(INCLUDE_SDL_HEADERS_IMAGE) $(INCLUDE_SDL_HEADERS_TTF) $(INCLUDE_SDL_HEADERS_MIXER) $(PATHFRAMEWORK) $(FRAMEWORK_SDL) $(PATHFRAMEWORK) $(FRAMEWORK_SDL_IMAGE) $(PATHFRAMEWORK) $(FRAMEWORK_SDL_MIXER) $(PATHFRAMEWORK) $(FRAMEWORK_SDL_TTF) -o $(BUILD_DIR)/$(OBJ_NAME) -v && rsync -avr $(SRC_DIR)/assets $(BUILD_DIR)/
